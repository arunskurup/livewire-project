<div class="flex flex-col w-[300px] mx-auto py-16" >
   ...
   <div class="py-6">
    ...
    <div class="flex gap-4 justify-between items-center py-1">
        <input type="checkbox" {{$item->completed ? 'checked':''}} wire:change="toggleTodos({{$item->id}})" name="" id="">
        <label class="flex-1 {{$item->completed ? 'line-through':''}}">{{$item->todo}}</label>
        <button wire:click="deleteTodos({{$item->id}})">
